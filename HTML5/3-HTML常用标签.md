...
2.从h1~h6重要性递减，h1最重要，h6最不重要

3.h1在网页中的重要性仅次于title标签，一般情况下一个页面中只会有一个h1

4.一般情况下标题标签只会使用到h1~h3，h4~h6很少用
...
2.默认情况下，文本在一个段落中会根据浏览器窗口的大小自动换行。

3.p也是一个块元素
但 p 里面不能放块级元素，特别是 p 不能放div
...
```
div标签   块元素，没有语义，用来表示一个盒子，是我们主要的布局元素
...
strong 还有强调的意思，语义更强烈。

<i></i> <em></em>  字体倾斜
em 标签用于表示语音语调的一个加重（更推荐使用）
...
```
块元素（block element）（<h1>~<h6>、<p>、<div>、<ul>、<ol>、<li>）
	- 块级元素各占据一行，垂直方向排列。
	- 在网页中一般通过块元素来对页面进行布局
	- 高度，宽度、外边距以及内边距都可以控制
	- 宽度默认是容器（父级宽度）的100%
...
行内元素（inline element）（<a>、<strong>、<b>、<em>、<i>、<del>、<s>、<ins>、<u>、<span>）
...
超链接 a 是也是行内元素，在a标签中可以嵌套除它自身外的任何行内元素
...
1.在开发中可以将#作为超链接的路径（href）的展位符使用 - 空链接
<a href="#">这是一个新的超链接</a>
- 空链接默认回到页面顶部！

2.锚点定位 - 可以跳转到页面的指定位置，只需将href属性设置 #目标元素的id属性值
    id属性（唯一不重复的）
        - 每一个标签都可以添加一个id属性
...
2.绝对路径
完整文件目录路径 “D:\web\img\logo.gif” 或 完整的网络地址，例如 “http://www.itcast.cn/images/logo.gif”
注意:
	绝对路径用的较少，但要注意它的写法特别是符号 \ 并不是 相对路径的 /
```
...
3.img这种元素属于行内块（替换）元素（块和行内元素之间，具有两种元素的特点）
    属性：
...
注:
 ...
 3) 无序列表会带有自己样式属性，但一般会去除默认样式，使用css设置

3.自定义列表，使用dl标签来创建一个自定义列表
    使用dt来表示定义的内容
    使用dd来对内容进行解释说明
<dl>
  <dt>名词1</dt>
...
3.(一般用这种方法)除了通过src来指定外部文件的路径以外，还可以通过内嵌source标签来指定文件的路径
此方法可兼容所有浏览器！
实例:
<audio controls>
...
4.使用video标签来向网页中引入一个视频
	- 使用方式和audio基本上是一样的
	特有属性:（其他属性同音频）
    preload  规定是否预加载视频(如果有了autoplay就忽略该属性),auto(预先加载视频)/none (不应加载视频)
	poster  加载等待的画面图片,设置图片url
	muted  静音播放
实例:
...
<iframe
      frameborder="0"
      src="https://v.qq.com/txp/iframe/player.html?vid=b00318l66nt"
